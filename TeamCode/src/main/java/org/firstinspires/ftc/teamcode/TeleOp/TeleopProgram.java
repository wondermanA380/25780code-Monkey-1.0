...
@TeleOp(name="teleop for red alliance", group="OpMode")
public class TeleopProgram extends LinearOpMode{
                ...
                if (gamepad2.b) {
                    ...
                    while (isOuttakeSlideBusy && opModeIsActive()) {
                        ...
                        if (outtakeSlide.getCurrentPosition() < 15) {
                            isOuttakeSlideBusy = false;
                        } else if (outtakeTouch.isPressed() || gamepad2.dpad_left) {
                            outtakeSlide.setPower(0);
                            outtakeSlide.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                            isOuttakeSlideBusy = false;
                        }
                    ...
                    while (isIntakeSlideBusy && opModeIsActive()) {
                        ...
                        if (intakeSlide.getCurrentPosition() < 15) {
                            isIntakeSlideBusy = false;
                        } else if (intakeTouch.isPressed() || gamepad2.dpad_right) {
                    ...
                    if(intakeSlide.getCurrentPosition() < 15 || intakeTouch.isPressed()){
                        ...
                        while (colorSensor.red() > 500 || colorSensor.alpha() > 500) {
                        ...
                        outtakeTimer.reset();
                        while (outtakeTimer.milliseconds() <= 500) {
                            robotMovemnt(.75f);
                        }
    ...
    public void intakeHandling() {
        ...
        ///////put in blue teleop program
                /*if(colorSensor.red() > 500 && colorSensor.alpha() < 500){ red rejection(make seperate program for this)
                        leftclaw.setPower(0.9);
                        rightclaw.setPower(-0.9);
